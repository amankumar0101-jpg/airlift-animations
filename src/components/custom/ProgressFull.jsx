...
const ProgressBar = () => {
  ...
  // controls for the bottom "completed" row collapse + removal
  const [showBottom, setShowBottom] = useState(true); // whether to render at all
  const [removingBottom, setRemovingBottom] = useState(false); // triggers height -> 0
  const removeTimeoutRef = useRef(null);

  const handleCloseBottom = () => {
    if (!showBottom || removingBottom) return;
    // collapse height to 0 (CSS transition will do the animation)
    setRemovingBottom(true);
    // after 3s remove from DOM (display: none)
